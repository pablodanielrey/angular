...
class LogsDAO:
    ...
    @staticmethod
    def findByUserId(con, ids):
        assert isinstance(ids, list)
        cur = con.cursor()
        try:
            cur.execute('select * from assistance.attlogs where user_id in %s', (tuple(ids),))
            return [ LogsDAO._fromResult(r) for r in cur ]
...
class AssistanceModel:
    ...
    def getAttLogsByUserId(self, con, ids):
        assert isinstance(ids, list)
        return LogsDAO.findByUserId(con, ids)
