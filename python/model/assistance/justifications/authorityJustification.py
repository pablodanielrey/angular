...
class AuthorityJustificationDAO(AssistanceDAO):
    ...
    def _createSchema(cls, con):
        ...
        try:
            sql = """
              ...
              create table IF NOT EXISTS assistance.justification_authority (
    ...
    def _fromResult(cls, con, r):
        ...
        c = AuthorityJustification(date, r['user_id'], r['owner_id'])
        c.id = r['id']
        c.notes = r['notes']
    ...
    def persist(cls, con, c):
        ...
        try:
            ...
            if len(c.findById(con, [c.id])) <=  0:
                r = c.__dict__
                cur.execute('insert into assistance.justification_authority (id, user_id, owner_id, date, notes) '
                            'values ( %(id)s, %(userId)s, %(ownerId)s, %(date)s, %(notes)s)', r)
            else:
                r = c.__dict__
                cur.execute('update assistance.justification_authority set user_id = %(userId)s, owner_id = %(ownerId)s, '
    ...
    def findById(cls, con, ids):
        ...
        cur = con.cursor()
        try:
            cur.execute('select * from assistance.justification_authority where id in %s', (tuple(ids),))
    ...
    def findByUserId(cls, con, userIds, start, end):
        ...
        try:
            ...
            cur.execute('select * from assistance.justification_authority where user_id  in %s and date BETWEEN %s AND %s', (tuple(userIds), sDate, eDate))
            return [ cls._fromResult(con, r) for r in cur ]
        finally:
            cur.close()
