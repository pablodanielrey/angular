...
class EvaluationJustificationDAO(AssistanceDAO):
    ...
    def persist(cls, con, c):
        ...
        try:
            ...
            if len(c.findById(con, [c.id])) <=  0:
                r = c.__dict__
                cur.execute('insert into assistance.justification_evaluation (id, user_id, owner_id, date, notes) '
    ...
    def findByUserId(cls, con, userIds, start, end):
        ...
        try:
            ...
            cur.execute('select * from assistance.justification_evaluation where user_id  in %s and date BETWEEN %s AND %s', (tuple(userIds), start, eDate))
            return [ cls._fromResult(con, r) for r in cur ]
        finally:
            cur.close()
