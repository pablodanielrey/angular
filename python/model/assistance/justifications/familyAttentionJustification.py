...
class FamilyAttentionJustificationDAO(AssistanceDAO):
    ...
    def _createSchema(cls, con):
        ...
        try:
            sql = """
              ...
              create table IF NOT EXISTS assistance.justification_family_atention (
    ...
    def _fromResult(cls, con, r):
        j = FamilyAttentionJustification()
        j.id = r['id']
        j.userId = r['user_id']
    ...
    def persist(cls, con, j):
        ...
        try:
            ...
            if len(j.findById(con, [j.id])) <=  0:
                r = j.__dict__
                cur.execute('insert into assistance.justification_family_atention (id, user_id, owner_id, jstart, jend, number, notes) '
                            'values (%(id)s, %(userId)s, %(ownerId)s, %(start)s, %(end)s, %(number)s, %(notes)s)', r)
            else:
                r = j.__dict__
                cur.execute('update assistance.justification_family_atention set user_id = %(userId)s, owner_id = %(ownerId)s, '
    ...
    def findById(cls, con, ids):
        ...
        try:
            ...
            cur.execute('select * from assistance.justification_family_atention where id in %s',(tuple(ids),))
    ...
    def findByUserId(cls, con, userIds, start, end):
        ...
        try:
            ...
            cur.execute('select * from assistance.justification_family_atention where user_id in %s and '
