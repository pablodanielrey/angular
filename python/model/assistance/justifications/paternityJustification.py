# -*- coding: utf-8 -*-
'''
    implementa la justificación de Paternidad (Nacimiento)
    dentro del registry debe existir una sección :

    [paternityJustification]
    continuousDays = True
...
class PaternityJustificationDAO(DAO):
    ...
    def _createSchema(cls, con):
        ...
        try:
            sql = """
              ...
              create table IF NOT EXISTS assistance.justification_paternity (
    ...
    def findById(con, ids):
        ...
        try:
            ...
            cur.execute('select * from assistance.justification_paternity where id in %s',(tuple(ids),))
            return [ PaternityJustificationDAO._fromResult(con, r) for r in cur ]
        finally:
            cur.close()
...
class PaternityJustification(RangedJustification):
