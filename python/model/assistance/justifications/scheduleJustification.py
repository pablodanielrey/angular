...
class ScheduleJustificationDAO(DAO):
    ...
    def _fromResult(cls, con, r):
        c = ScheduleJustification(r['user_id'], r['owner_id'], r['date'])
        c.id = r['id']
    ...
    def findByUserId(cls, con, userIds, start, end):
        ...
        try:
            ...
            cur.execute('select * from assistance.justification_schedule where user_id  in %s and date BETWEEN %s AND %s', (tupe(userIds), sDate, eDate))
            return [ cls._fromResult(con, r) for r in cur ]
        finally:
            cur.close()
