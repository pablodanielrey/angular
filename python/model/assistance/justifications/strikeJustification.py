...
class StrikeJustificationDAO(AssistanceDAO):
    ...
    def findByUserId(cls, con, userIds, start, end):
        ...
        try:
            ...
            cur.execute('select * from assistance.justification_strike where user_id  in %s and date BETWEEN %s AND %s', (tuple(userIds), start, eDate))
            return [ cls._fromResult(con, r) for r in cur ]
        finally:
            cur.close()
...
class StrikeJustification(SingleDateJustification):
    ...
    def __init__(self, date = None, userId = None, ownerId = None):
        ...
        self.identifier = StrikeJustification.identifier
