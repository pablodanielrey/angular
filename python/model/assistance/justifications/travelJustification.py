# -*- coding: utf-8 -*-
'''
    implementa la justificación de Viaje
...
class TravelJustificationDAO(AssistanceDAO):
    ...
    def _createSchema(cls, con):
        ...
        try:
            sql = """
              ...
              create table IF NOT EXISTS assistance.justification_travel (
    ...
    def _fromResult(cls, con, r):
        j = TravelJustification(r['user_id'], r['owner_id'], r['jstart'], 0)
        j.id = r['id']
        j.end = r['jend']
        j.notes = r['notes']
    ...
    def persist(cls, con, j):
        ...
        try:
            ...
            if len(j.findById(con, [j.id])) <=  0:
                r = j.__dict__
                cur.execute('insert into assistance.justification_travel (id, user_id, owner_id, jstart, jend, notes) '
                            'values (%(id)s, %(userId)s, %(ownerId)s, %(start)s, %(end)s, %(notes)s)', r)
            else:
                r = j.__dict__
                cur.execute('update assistance.justification_travel set user_id = %(userId)s, owner_id = %(ownerId)s, '
...
class TravelJustification(RangedJustification):

    dao = TravelJustificationDAO
    registry = inject.instance(Registry).getRegistry('travelJustification')
    identifier = 'Viaje'

    def __init__(self, userId = None, ownerId = None, start = None, days = 0):
        super().__init__(start, days, userId, ownerId)
        self.identifier = TravelJustification.identifier
