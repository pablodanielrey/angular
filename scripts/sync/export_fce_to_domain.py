...
def modifyUser(l,dni,name,lastname,username,password,result):
    ...
    print("Modificando usuario {}".format(dni))
    ...
    if 'gidNumber' in attrs:
        ...
        print("Error, no posee gidNumber el usuario {}".format(username))
        exit(1)

    if 'uidNumber' in attrs:
        uidNumber = attrs['uidNumber'][0]
    else:
        print("Error, no posee uidNumber el usuario {}".format(username))
        exit(1)
    ...
    upass = unicode(password,'utf8')
    putf16 = upass.encode('utf-16le')
    pmd4 = hashlib.new( 'md4', putf16)
    pdigest = pmd4.digest()
    phex = pdigest.encode('hex')

    nt_password = phex.upper()
...
def createUser(l,dni,name,lastname,username,password):
    ...
    putf16 = password.encode('utf-16le')
    pmd4 = hashlib.new( 'md4', putf16)
    pdigest = pmd4.digest()
    phex = pdigest.encode('hex')
