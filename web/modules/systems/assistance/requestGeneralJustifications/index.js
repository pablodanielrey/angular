...
app.controller('RequestGeneralJustificationsCtrl', ["$scope", "$timeout", "Assistance", "Module", "Notifications", "Users",  "Utils", function($scope, $timeout, Assistance, Module, Notifications, Users, Utils) {
  ...
  $scope.model = {
    ...
    requestedJustificationsFiltered: null,   //requerimientos filtrados de las justificaciones autorizadas
  ...
  $scope.loadRequestedJustifications = function() {
    ...
    Assistance.getJustificationRequestsToManage(['APPROVED'],"TREE",
      function(requestedJustifications) {
        ...
        $scope.filterRequestedJustifications();
      },
      function(error) {
   ...
   $scope.filterRequestedJustifications = function(){

    $scope.model.requestedJustificationsFiltered = [];

    for (var i = 0; i < $scope.model.requestedJustifications.length; i++) {
      if($scope.isAuthorizedJustification($scope.model.requestedJustifications[i].justification_id)){
