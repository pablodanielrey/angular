...
app.controller('UsersAssistanceManagementCtrl', ["$scope", "$timeout", "$window", "$wamp", "Assistance", "Notifications", "Users",  "Utils", "Session", "Office", function($scope, $timeout, $window, $wamp, Assistance, Notifications, Users, Utils, Session, Office) {
  ...
  $scope.loadAuthorizedUsers = function(){
    ...
    Office.getUserInOfficesByRole(sessionUserId, 'realizar-solicitud', true,
      function(users) {

        $scope.model.users = [];

        // eliminamos el usuario jefe asi no se autoautoriza pedidos a si mismo.
  ...
  $timeout(function() {
   ...
   function JustificationsRequestsUpdatedEvent(args) {
     data = args[0];
     $scope.$broadcast('JustificationsRequestsUpdatedEvent',data);
    $scope.loadRequestedJustifications();
   }

   function JustificationStatusChangedEvent(args) {
     data = args[0];
